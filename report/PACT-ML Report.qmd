...
Contrary to popular ML course opinion (ironically: "main takeaway from the class is that Random Trees always win"), for text data, Random Forest models are not a suitable class. We included them in the reporting because a) the model already ran and b) it is important to rank the base models performance.

### Feature importance

When comparing the results across models, it is important to look at the feature importance. In general, the interpretability of a model is important to understand and assert that the model is doing what it is intended to do. For working with text data however,  cross-mission and cross-label comparisons can be helpful in assessing the model too, as the validity
