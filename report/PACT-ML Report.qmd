...
@fig-label-distr shows that the overall label distributions stays comparable between the full PACT data set and the subset parsed for analysis. Notably, 'CivilSocietyAssistance' is carried out heavier in the selected mission reports, while there was less use of force. This finding makes sense, as the use of force is only mandated for very specific third and fourth generation UNPKOs, also referred to as "robust" peacekeeping missions. These missions were mostly deployed in armed conflicts in Africa. Since data of PACT 1.0, which covers Africa, was not available, the reported activities in our subset rely heavily on the UNOMIG mission (@fig-label-per-mission). This potentially leads to results that rely on mission-specific wording in our models.
...
Word clouds for each category Most distinctive terms per category (CountVectorizer from scikit-learn)
...
if ("paragraph" %in% colnames(merged_data)) {
  par(mfrow = c(2, 3))  # Adjust grid layout based on number of categories
  for (category in target_categories) {
    create_category_wordcloud(merged_data, category)
  }
  par(mfrow = c(1, 1))  # Reset layout
} else {
  cat("Paragraph text not available for word clouds.\n")
}
...
if ("paragraph" %in% colnames(merged_data)) {
  ...
  ggplot(distinctive_terms, aes(x = reorder_within(word, tf_idf, category), 
    ...
    labs(
      title = "Most Distinctive Terms by Category",
      x = NULL,
      y = "TF-IDF Score"
    ) +
    theme_minimal() +
  ...
  cat("Paragraph text not available for distinctive terms analysis.\n")
}
...
We opt for a tri-fold modeling approach. First, the baseline models using 'CountVectorizer' for tokenizations are calculated, using Logistic Regression, Balanced Logistic Regression and a Random Forest Tree Classifier for subsequent classifications. Second, the same models are employed using TF-IDF to tokenize documents. Thirdly, a pre-trained BERT model is fitted to the data.
