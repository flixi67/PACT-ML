...
```{r}
#| label: tbl-results
#| caption: "Model performance metrics and uncertainty"

# import all .csv from out

# calculate means for everything and variance for F1 macro

print("placeholder (TBD)")

```

Elaborate on False Positives vs False Negatives, pick best model for each preference case. Pick overall best model and write summary of models. Highlight best model in text ###
...
Contrary to popular ML course opinion (ironically in our WhatsApp group: "main takeaway from the class is that Random Forests always win"), for text data, Random Forest models are not a suitable class. We included them in the reporting because a) the model already ran and b) it is important to better rank the base models performance, which is quite good regarding the simplicity of the modeling approach and tokkenization progress.

### Feature importance

When comparing the results across models, it is important to look at the feature importance to better determine differences between models. In general, the interpretability of a model is important to understand and assert that the model is doing what it is intended to do. For working with text data however,  cross-mission and cross-label comparisons can be helpful in assessing the model too, as the validity...
...
- expected improvements from this for the BoW and TF-IDF (balanced) logistic models: 5-15% in F1 score and 3-10% in missclassification rate.
